...
\begin{frame}
  \frametitle{The goal of Wiener filtering}
  Reduce degradation and noise in an image, audio signals, etc.
  \begin{figure}
    \centering
...
\begin{frame}
  ...
  Suppose you have a signal \(S(\bm{t})\) in the time domain (whatever ``time''
  is: actual time, point in space, pixel of an image, etc), blurred by a shift
  invariant function \(B(\bm{t})\) and with an additive noise \(N(\bm{t})\)
  \begin{equation*}
    X(\bm{t}) = (B * S)(\bm{t}) + N(\bm{t})
  \end{equation*}
  Its Fourier transform in the frequency domain is denoted by \(\hat{X}(\bm{f})
  = \mathcal{F}(X)(\bm{f})\).  We want to find an appropriate filter
  \(W(\bm{f})\) such that the function \(Y(\bm{f}) = W(\bm{f})\hat{X}(\bm{f})\)
  is as much close as possible to the Fourier transform of the original signal
  \(\hat{S}(\bm{f}) = \mathcal{F}(S)(\bm{f})\), i.e. we want to minimize the
  quantity
  \begin{equation*}
  ...
  \end{equation*}
  This is the \alert{Wiener filter} function
\end{frame}
